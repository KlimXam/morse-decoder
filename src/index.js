...
function decode(expr) {
    ...
    while (expr.length > 0) { // пока длина строки больше 0 => продолжаем   декодировать    буквы
        ...
        for (let i = 0; i < str.length; i++) { // пробегаем по первым 10 символам
            sign += str[i]; // добавляем в переменную знак
            if (sign === '10') { // если знак равен 10
                letter += '.'; 
                sign = '';
                continue;
            } else if (sign === '11') { // если знак равен 11
                letter += '-';
                sign = '';
                continue;
            } else if (sign === '00') { // если знак равен 00
                sign = '';
                continue;
            }
        }
    ...
    return result.reduce((before, key) => { // преобразуем массив в строку
